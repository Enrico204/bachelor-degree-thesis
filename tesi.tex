...
Il movimento delle placche nei margini, tuttavia, non è libero da attrito: a seconda del materiale, della profondità e della conformazione della roccia presente nella faglia, la faglia stessa tende ad \textbf{opporsi} al movimento delle placche, accumulando energia da attrito, la quale viene rilasciata repentinamente (al superamento della massima forza d'attrito) come energia meccanica (le cosìdette \textit{onde sismiche}) generando un \textit{terremoto} (o sisma).

\begin{figure}[h]
\label{fig:mappafaglie}
\caption{Mappa delle faglie italiane attive}
\centering
\includegraphics[width=13cm]{introduzione/mappa_faglie_italiane}
\end{figure}


\section{Analisi e misura dei terremoti}

Per misurare l'energia sprigionata da un evento sismico si utilizzano i \textbf{sismometri}, ovvero apparati dotati di accelerometri molto precisi, filtri e amplificatori in grado di rilevare l'accelerazione sulle tre componenti (X-Y-Z) e produrre un \textit{sismogramma} (Figura \ref{fig:sismogramma}).
